...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const results: {
      balancetes?: { deletedFiles: number; deletedRecords: number };
      dePara?: { deletedFiles: number };
    } = {};

    // Limpar Balancetes
    if (dataTypes?.includes("balancetes")) {
      if (!startPeriod || !endPeriod) {
        return NextResponse.json(
          { error: "startPeriod e endPeriod são obrigatórios para limpar balancetes" },
          { status: 400 }
        );
      }
    ...
    // Limpar De x Para
    if (dataTypes?.includes("dePara")) {
      const deletedDePara = await prisma.companyFile.deleteMany({
        where: {
          companyId,
          type: "DE_PARA",
        },
      ...
      results.dePara = {
        deletedFiles: deletedDePara.count,
      };
    }
